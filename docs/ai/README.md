...
护栏（Guardrails）是在 Agent 执行链路上设置的检查点，用于拦截不安全的输入、输出和操作。按所处位置可分为三层：

### 输入护栏（Before Agent）

- **Prompt 注入检测**：识别"忽略之前的指令"等越狱话术。除了用户直接输入，更要警惕**间接注入**——RAG 检索到的文档、网页、工具返回结果中夹带的恶意指令
- **有害内容过滤**：暴力、恐怖、违法等内容
- **PII 检测**：避免敏感信息进入模型或被写入日志、记忆

### 输出护栏（After Agent）

- **有害内容 / PII 过滤**：防止模型输出违规内容或泄露敏感信息
- **格式与事实校验**：结构化输出校验、幻觉检测（参见[评估](#评估)中的 Faithfulness）

### 工具调用护栏

- **最小权限**：只暴露完成任务所需的工具，只读/写权限分开
- **参数校验**：校验调用参数是否在白名单或合法范围内
- **人工审批**：转账、删除、发邮件等高风险操作前走 Human in the Loop（参见[中断](#中断)）

### PII 处理策略

检测身份证号、手机号、邮箱、信用卡号等敏感信息后，常见处理方式（以 LangChain `PIIMiddleware` 为例）：

| 策略 | 行为 | 是否保留可辨识性 | 典型场景 |
|---|---|---|---|
| redact | 整段替换为 `[REDACTED_类型]` | 否 | 合规、日志脱敏 |
| mask | 部分遮盖，保留少量尾部可读信息 | 部分 | 客服界面、人工核对 |
| hash | 替换为确定性哈希 `<类型_hash:摘要>` | 是（假名化，可关联同一值） | 分析、排错 |
| block | 检测到即抛出异常（`PIIDetectionError`） | N/A | 严禁出现该类信息 |

> 手机号、身份证号取值空间小，普通哈希可被穷举还原，hash 策略应使用带密钥的 HMAC

### 实现方式

| 方式 | 优点 | 缺点 |
|---|---|---|
| 基于规则（正则、关键词、黑白名单） | 快、便宜、结果确定 | 容易被改写、编码绕过 |
| 基于模型（LLM 语义判断、Llama Guard 等安全分类模型） | 能理解语义，覆盖面更广 | 慢、贵、存在误判 |

> 实践中通常分层组合：规则先行拦截明显违规且成本低，模型兜底处理语义层面的风险
...
Agent 的 Token 消耗随调用轮数和上下文长度快速放大，常见的降本手段：

- **模型路由**：简单任务交给便宜的小模型，只有复杂任务才用大模型
- **Prompt Cache**：复用重复的前缀，避免每轮都按原价重新计算
- **上下文压缩**：对历史对话做摘要、裁剪无关的工具返回结果（参见[记忆](#记忆)）
- **限制输出长度**：设置 `max_tokens`，要求结构化的简洁输出
- **Batch API**：离线、非实时任务走批处理接口，价格通常有较大折扣

### 模型路由

| 方式 | 做法 | 特点 |
|---|---|---|
| 规则路由 | 按任务类型、输入长度、用户等级等规则分配模型 | 简单可控，但规则难以覆盖所有情况 |
| 分类器路由 | 用小模型或专门训练的 Router 先判断难度，再选择模型 | 更智能，但路由本身也有成本和误判 |
| 级联（Cascade） | 先用小模型回答，校验不通过或置信度低时再升级到大模型 | 大部分请求由小模型完成，但失败的请求延迟更高 |

> 切换模型会导致 Prompt Cache 失效，同一会话内应尽量固定模型

### Prompt Cache

原理是**前缀匹配**：只有从头开始完全一致的部分才能命中缓存，中间任何一个字符不同，其后的内容全部失效。这一点类似 MySQL 联合索引的最左前缀原则。

提高命中率的做法：

- **静态内容在前，动态内容在后**：按 工具定义 → System Prompt → 固定示例 → 历史对话 → 本轮输入 的顺序组织
- **不要在前缀中放入易变内容**：比如 System Prompt 里的当前时间、请求 ID、随机排序的工具列表、键顺序不固定的 JSON
- **对话历史只追加不修改**：中途改写或删除早期消息会让后续缓存全部失效
- **注意 TTL**：缓存有过期时间（如 Anthropic 默认 5 分钟，每次命中会刷新）。间隔较长的场景可以定时发送轻量请求保活，或者选用更长的 TTL，前提是保活成本低于缓存重建成本

> 各厂商实现不同：OpenAI 自动缓存，Anthropic 需要通过 `cache_control` 显式标记缓存断点，且都有最小缓存长度限制。命中情况可通过响应中的 `usage` 字段（如 `cached_tokens` / `cache_read_input_tokens`）观察

## Gateway

LLM Gateway 位于应用和模型供应商之间，用统一接口屏蔽不同厂商 API 的差异，并集中处理横切关注点：

- **统一接口**：以 OpenAI 兼容格式调用各家模型，切换供应商无需改业务代码
- **路由与容灾**：负载均衡、失败重试、跨模型/跨供应商 fallback
- **成本管控**：虚拟 Key、按团队/项目设置预算与限流、跨供应商成本统计
- **可观测性**：统一记录请求日志、延迟、Token 用量
- **附加能力**：响应缓存、护栏（参见[护栏](#护栏)）

常用方案：

| 方案 | 特点 |
|---|---|
| LiteLLM | 开源，Python SDK + Proxy Server，可自部署，支持的模型供应商很多 |
| Portkey | 提供开源 Gateway 和托管平台，内置虚拟 Key、预算、fallback、护栏等企业级功能 |
